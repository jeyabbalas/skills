...
- **playground.html**: needs a local server, because its engine runs in a web worker and browsers block workers and module scripts on `file://`. From the package root: `python3 -m http.server 8000`, then `http://localhost:8000/playground.html`. Serve the whole package directory, not the page alone — `assets/vendor/` has to sit beside it. Opened from disk anyway, the page shows exactly these instructions instead of a broken screen — designed degradation, not a bug.
- **When the engine cannot start** — an old browser, or a network that blocks its one download — the playground renders the toy rows as a static table instead, every violation marked on its cell with the same rule text the grid would have shown. The steward loses sorting, filtering and column stats, not the findings. Say that plainly rather than calling the page broken.
- Hosting the pages somewhere with a strict content-security policy: the playground needs `'unsafe-eval'` (its validator compiles schemas), `'wasm-unsafe-eval'`, `worker-src 'self' blob:` (its database engine starts a nested worker through a blob), and `connect-src` for `cdn.jsdelivr.net`; semantic search needs `connect-src` for `cdn.jsdelivr.net` and `huggingface.co`. Where those are refused, the dictionary and its keyword search are untouched — ship the dictionary page alone and let `tools/validate.py` do the checking.
