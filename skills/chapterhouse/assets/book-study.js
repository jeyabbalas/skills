/* book-study.js — shared behavior for chapterhouse study workspaces.
...
(function () {
  ...
  /* ---------- live filter (concepts index, decks, shelf) ---------- */
  ...
  /* ---------- expand / collapse all (chapter notes) ---------- */
  ...
  /* ---------- deck answers: reveal/hide all + the print-key toggle ---------- */

  function initAnswerTools() {
    document.querySelectorAll('[data-answers]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        var open = btn.getAttribute('data-answers') === 'open';
        document.querySelectorAll('details.answer').forEach(function (d) { d.open = open; });
      });
    });
    var toggle = document.querySelector('[data-print-key]');
    if (toggle) {
      toggle.addEventListener('click', function () {
        var on = document.body.classList.toggle('print-key');
        toggle.setAttribute('aria-pressed', String(on));
        toggle.textContent = on ? 'Print: with answer key' : 'Print: questions only';
      });
    }
  }

  /* ---------- print: open every <details>, then restore ----------
     Answers stay closed for a questions-only printout unless the print-key
     toggle put print-key on <body> (the CSS hides them either way, but an
     opened-then-hidden answer would still leak into non-deck print styles). */

  function openForPrint() {
    var withKey = document.body.classList.contains('print-key');
    document.querySelectorAll('details:not([open])').forEach(function (d) {
      if (!withKey && d.classList.contains('answer')) return;
  ...
  /* ---------- prerequisite map ----------
     ...
     href. Edge kinds: prereq | soft (soft renders dashed). */
