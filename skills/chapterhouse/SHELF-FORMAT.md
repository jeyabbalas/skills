...
- **`progress`** is `recited K/N` — chapters recited over chapters planned (skipped and skimmed chapters leave the denominator). While a chapter is mid-read it may append the current unit — `recited 0/8 · ch 1 §1.2` — a gist of BOOK.md's `next:`, never the authority.
